...
import (
	...
	"vicomova/internal/commerce/wire"
	"vicomova/pkg/config"
...
func init() {
	config.Init(constants.ServiceCommerce)
}
...
func main() {
	...
	svr := commerceService.NewServer(p.CommerceHandler, server.WithServiceAddr(tcpAddr))

	log.Info.Printf("Commerce RPC server starting on %s", addr)
	...
	registry := etcd.NewRegistry(cli, constants.ServiceCommerce, addr)
